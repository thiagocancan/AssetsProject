<div>
    ...
    <div class="mb-6 w-fit ml-3 text-zinc-500">
        <ul class="flex items-center space-x-6">
            ...
            <li>
                <button 
                    ...
                    class="hover:text-black cursor-pointer border-black
                        @if($activeTab === 'reviews') border-b text-black @endif">Reviews ({{ $assets->sum(fn($asset) => $asset->reviews->count()) }})</button>
            </li>
        </ul>
    </div>
            ...
            @foreach($assets as $asset)
                <div class="border border-zinc-300 rounded p-6 space-y-4">
                    <div class="flex justify-between">
                        <h2 class="font-semibold text-lg">{{ $asset->title }}</h2>
                        @for ($i = 0; $i < $asset->reviews->first()->rating; $i++)
                            ⭐
                        @endfor
                    </div>
                    @if($asset->reviews->first())
                        <p class="text-sm text-zinc-700">{{ $asset->reviews->first()->comment }}</p>
                    @endif
                    <div class="flex items-center space-x-3">
                        <img class="rounded-full size-6" src="https://ui-avatars.com/api/?name={{ urlencode($asset->reviews->first()->user->name) }}&background=random" alt="">
                        <h3 class="text-sm">{{ $asset->reviews->first()->user->name }} - {{ $asset->reviews->first()->created_at->diffForHumans() }}</h3>
                    </div>
                </div>
            @endforeach
        </div>
